...
const authZendesk = `Basic ${Buffer.from(`${process.env.ZENDESK_EMAIL}/token:${process.env.ZENDESK_TOKEN}`).toString('base64')}`
...
const getAttachments =async(ticket_id) =>{
    let attachments = []
    const response = await zendeskAPI.get(`/api/v2/tickets/${ticket_id}/comments?include=users`)
...
const getRequester=async(ticket_id) =>{
    const response = await zendeskAPI.get(`/api/v2/tickets/${ticket_id}.json?include=users`)
    return response.data.users.find(item => item.id === response.data.ticket.requester_id)
}
...
const main =async()=>{
    ...
    await fs.writeFile("fileImagem2.txt",binaryAttachments[0])
        .then(console.log)
        .catch(console.log)
